...
class EFMPretrainingTrainer:
    def __init__(
        ...
        *,
        ...
        self.train_state = {"epoch": 0, "file_index": 0, "global_step": 0}
    ...
    def train(self) -> None:
        ...
        for epoch in range(epochs):
            ...
            for file_offset, path in enumerate(self.data_assets.train_paths):
                ...
                if self.runtime.is_main:
                    self.logger.info(
                        ...
                        epochs,
                        file_index,
                        len(self.data_assets.train_paths),
                        path,
                    )
                ...
                try:
                    data_bundle = build_data_bundle_for_path(
                        ...
                        num_files=len(self.data_assets.train_paths),
                    )
                    ...
                    try:
                        progress = tqdm(
                            ...
                            desc=f"efm epoch {epoch + 1}/{epochs} adata {file_index}/{len(self.data_assets.train_paths)}",
                        )
...
def main() -> None:
    ...
    try:
        ...
        paths = prepare_experiment_paths(runtime=runtime, resume_path=None)
        paths = replace(
            paths,
            log_file=paths.logs / "efm_pretrain.log",
            resume_state_file=paths.checkpoints / "efm_train_state.pt",
        )
        ...
        efm = _build_efm(config=config, data_assets=data_assets, assets=runtime_assets)
        if bool(config.get("efm", {}).get("init_from_sfm", True)):
        ...
        trainer = EFMPretrainingTrainer(
            ...
            config=config,
            assets=source_assets,
            model_package_dir=paths.model_package_dir,
        )
        trainer.train()
    finally:
        cleanup_distributed()
