...
from ..models.utils import FactorState, reparameterize, expand_u
...
class VariationalEncoder(nn.Module):
    ...
    def _predict_tg_from_factors(
        ...
        u_eff = u * u_score
        v_eff = v * v_score
        tmp = torch.einsum("cfm,cfe->cme", u_eff, expr_tf)
        pred_tg = torch.einsum("cgm,cme->cge", v_eff, tmp)
        return pred_tg
    ...
    def forward(self, x, binary_tf, binary_tg, u, v, u_score, v_score):
        ...
        if v.shape[0] != x.shape[0] or v.shape[1] != expr_tg.shape[1]:
            raise ValueError(
...
class ExprModeling(nn.Module):
    ...
    def _apply_a_with_factors(self, h: torch.Tensor, u_full: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
        ut_h = torch.bmm(u_full.transpose(1, 2), h)
        return torch.bmm(v, ut_h)
    ...
    def _solve_with_factors(
        ...
        u_full = expand_u(u * u_score, binary_tf_sel)
        v_eff = v * v_score
        h = z_proj
