...
* Optionally keep only genes found in `assets/tokenizer/vocab.json`.
* For mouse downloads, remap `feature_id` and `feature_name` through `assets/resources/homologous.csv` so the exported files align with the human token vocabulary while preserving original mouse feature columns as `source_feature_id` and `source_feature_name`.
...
bash data/download_partition.sh \
  ...
  assets/resources/homologous.csv
```
...
python3 data/check_and_redownload_h5ad.py \
  ...
  --token-dict-path assets/tokenizer/vocab.json \
  --homology-path assets/resources/homologous.csv \
